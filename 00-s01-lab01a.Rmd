...
Which reads as sum all the values from i (the first value) to n (the last value) and then divide that by the total number of values (N). If we start to fill in the information from above we get:

$$\overline{X} = \frac{`r dat %>% filter(Participants == 1) %>% pull(X)` + 50000`r dat %>% filter(Participants == 2) %>% pull(X)` + `r dat %>% filter(Participants == 3) %>% pull(X)` +`r dat %>% filter(Participants == 4) %>% pull(X)` +`r dat %>% filter(Participants == 5) %>% pull(X)` +`r dat %>% filter(Participants == 6) %>% pull(X)` +`r dat %>% filter(Participants == 7) %>% pull(X)` +`r dat %>% filter(Participants == 8) %>% pull(X)` +`r dat %>% filter(Participants == 9) %>% pull(X)` +`r dat %>% filter(Participants == 10) %>% pull(X)`}{`r dim(dat)[1]`}$$
...
which can be read as the value ($X$) minus the mean value ($\overline{X}$), divided by the standard deviation ($s_{X}$).  For example, if we look at Participant 1:
