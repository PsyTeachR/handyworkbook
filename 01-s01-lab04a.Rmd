...
where the $\sum_i^n$ translates as sum (i.e. add up) the values of $(x_i - \bar{x})(y_i - \bar{y})$ for each pair of observations in your data. But to do that we first need to know the mean of x $\bar{x}$ and the mean of y, $\bar{y}$. Now we know that the mean of a value is the sum of all the values of that variable divided by the number of values - also written as $\bar{x} = \frac{\sum_i^n x_i}{n}$. So the mean of x is:
...
$$\bar{x} = \frac{`r dat_o %>% pull(HeadSize) %>% sum()`}{`r dim(dat_o)[1]`} = `r dat_o %>% pull(HeadSize) %>% sum()/dim(dat_o)[1]`$$
...
$$\bar{y} = \frac{`r dat_o %>% pull(IQ) %>% sum()`}{`r dim(dat_o)[1]`} = `r dat_o %>% pull(IQ) %>% sum()/dim(dat_o)[1]`$$

Ok great so we now have enough information to start filling in the covariance formula. First, however, before looking at it for all participants, let's look at this part, $(x_i - \bar{x})(y_i - \bar{y})$, for one participant just to make sure we understand what we are doing. And one thing to remember is that two sets of parentheses (i.e. brackets) side-by-side like that means to multiply them together. So, if we take Participant 1, then we see that:

* The mean of x is $\bar{x} = `r dat_o %>% pull(HeadSize) %>% sum()/dim(dat_o)[1]`$
* The mean of y is $\bar{y} = `r dat_o %>% pull(IQ) %>% sum()/dim(dat_o)[1]`$
...
P~1~ = $(x_i - \bar{x})(y_i - \bar{y}) = (`r x1` - `r xm`)\times(`r y1` - `r ym`) = (`r x1 - xm`)\times(`r y1 - ym`) = `r (x1 - xm)*(y1 - ym)`$

and we can do that for all participants which would look like:

* P~2~ = $(x_i - \bar{x})(y_i - \bar{y}) = (`r x2` - `r xm`)\times(`r y2` - `r ym`) = `r x2 - xm`\times`r y2 - ym` = `r (x2 - xm)*(y2 - ym)`$
* P~3~ = $(x_i - \bar{x})(y_i - \bar{y}) = (`r x3` - `r xm`)\times(`r y3` - `r ym`) = `r x3 - xm`\times`r y3 - ym` = `r (x3 - xm)*(y3 - ym)`$
* P~4~ = $(x_i - \bar{x})(y_i - \bar{y}) = (`r x4` - `r xm`)\times(`r y4` - `r ym`) = `r x4 - xm`\times`r y4 - ym` = `r (x4 - xm)*(y4 - ym)`$
* P~5~ = $(x_i - \bar{x})(y_i - \bar{y}) = (`r x5` - `r xm`)\times(`r y5` - `r ym`) = `r x5 - xm`\times`r y5 - ym` = `r (x5 - xm)*(y5 - ym)`$
* P~6~ = $(x_i - \bar{x})(y_i - \bar{y}) = (`r x6` - `r xm`)\times(`r y6` - `r ym`) = `r x6 - xm`\times`r y6 - ym` = `r (x6 - xm)*(y6 - ym)`$
* P~7~ = $(x_i - \bar{x})(y_i - \bar{y}) = (`r x7` - `r xm`)\times(`r y7` - `r ym`) = `r x7 - xm`\times`r y7 - ym` = `r (x7 - xm)*(y7 - ym)`$
* P~8~ = $(x_i - \bar{x})(y_i - \bar{y}) = (`r x8` - `r xm`)\times(`r y8` - `r ym`) = `r x8 - xm`\times`r y8 - ym` = `r (x8 - xm)*(y8 - ym)`$
* P~9~ = $(x_i - \bar{x})(y_i - \bar{y}) = (`r x9` - `r xm`)\times(`r y9` - `r ym`) = `r x9 - xm`\times`r y9 - ym` = `r (x9 - xm)*(y9 - ym)`$
* P~10~ = $(x_i - \bar{x})(y_i - \bar{y}) = (`r x10` - `r xm`)\times(`r y10` - `r ym`) = `r x10 - xm`\times`r y10 - ym` = `r (x10 - xm)*(y10 - ym)`$
...
$$cov(x,y) = \frac{(x_1 - \bar{x})(y_1 - \bar{y})+(x_2 - \bar{x})(y_2 - \bar{y})+(x_3 - \bar{x})(y_3 - \bar{y})+(x_4 - \bar{x})(y_4 - \bar{y})+\\(x_5 - \bar{x})(y_5 - \bar{y})+(x_6 - \bar{x})(y_6 - \bar{y})+(x_7 - \bar{x})(y_7 - \bar{y})+(x_8 - \bar{x})(y_8 - \bar{y})+\\(x_9 - \bar{x})(y_9 - \bar{y})+(x_{10} - \bar{x})(y_{10} - \bar{y})}{n-1}$$
...
$$\sigma = \sqrt\frac{\sum_i^n(x_{i} - \bar{x})^2}{n-1}$$
